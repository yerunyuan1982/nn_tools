...
def _conv_transpose2d(raw, input, weight, bias=None, stride=1, padding=0, output_padding=0, groups=1, dilation=1):
    ...
    in_channel_count = input.size()[1]
    out_channel_count = x.size()[1]
    kernel_size = weight.size()[2:]
    if (groups > 1) and (groups == out_channel_count) and (in_channel_count == out_channel_count) and (bias is None):
        w = np.zeros((out_channel_count, in_channel_count, kernel_size[0], kernel_size[1]), np.float32)
        w0 = weight.cpu().data.numpy()
        for i in range(out_channel_count):
            np.copyto(w[i, i, ...], w0[i, ...])
        groups = 1
    else:
    ...
    layer = caffe_net.Layer_param(name=name, type='Deconvolution',
                                  bottom=[log.blobs(input)], top=[log.blobs(x)])
    layer.conv_param(out_channel_count, kernel_size, stride=_pair(stride), groups=groups,
                     pad=_pair(padding), dilation=_pair(dilation), bias_term=bias is not None)
    if bias is not None:
        layer.add_data(w, bias.cpu().data.numpy())
    else:
        layer.param.convolution_param.bias_term = False
        layer.add_data(w)
    log.cnet.add_layer(layer)
